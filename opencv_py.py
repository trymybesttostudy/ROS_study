...
def callback(data):
        ...
        for cnt in contours:
            ...
            if width * height > 100:
                ...
                if 0 not in box.ravel():
                    ...
                    posture.append([center_x, center_y, center[0], center[1], theta])
        ...
        for cnt in contours:
            ...
            if width * height > 100:
                ...
                if 0 not in box.ravel():
                    ...
                    posture.append([center_x_01, center_y_01, center[0], center[1], theta])
                # print(posture[sum-1])
                # print(box)
    ...
    pub_red.data = angle_red
    pub_brown.data = angle_brown
    pub_orange.data = angle_orange
    pub_blue.data = angle_blue
    pub_green.data = angle_green
    pub_purple.data = angle_purple
    pub_yellow.data = angle_yellow
    rospy.loginfo(pub_red)
    rospy.loginfo(pub_brown)
if __name__ == '__main__':
    rospy.init_node('sub_opencv', anonymous=True)
    bridge = CvBridge()

    # 实例化发布者对象
    pub_red_p = rospy.Publisher("red",Float64, queue_size=5)
    pub_brown_p = rospy.Publisher("brown",Float64, queue_size=5)
    pub_orange_p = rospy.Publisher("orange",Float64, queue_size=5)
    pub_blue_p = rospy.Publisher("blue",Float64, queue_size=5)
    pub_green_p = rospy.Publisher("green",Float64, queue_size=5)
    pub_purple_p = rospy.Publisher("purple",Float64, queue_size=5)
    pub_yellow_p = rospy.Publisher("yellow",Float64, queue_size=5)
    # 创建消息对象
    pub_red = Float64()
    pub_brown = Float64()
    pub_orange = Float64()
